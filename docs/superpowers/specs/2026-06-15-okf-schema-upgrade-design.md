...
created: 2026-06-01                                   # lore-wiki 확장
updated: 2026-06-01                                   # lore-wiki 확장 ≡ OKF `timestamp`(최종 수정). 필드명 유지, 매핑만 선언
...
| `tags` | OKF 표준 | 유지 |
| `updated` | lore-wiki 확장 | 개명하지 않음. 스키마에서 OKF `timestamp`(최종 수정)와 동치임을 선언 |
...
**대안(채택 안 함)**: `updated` 를 문자 그대로 `timestamp` 로 개명 → 269개 일괄 수정이 필요한데
의미 이득이 없어 비채택. 매핑 선언으로 충분하다.
...
- **"## OKF 계보" 단락 신설**: 본 스키마가 OKF v0.1 기반임을 명시하고 필드를 세 갈래로 구분해
  표로 정리 — ① OKF 표준 그대로(`type`/`title`/`description`/`tags`), ② OKF `timestamp` 에 대응하나
  필드명 유지(`updated`), ③ lore-wiki 순수 확장(`created`/`sessions`). OKF 의 구조 원칙
...
2. 이미 `title` 과 `description` 이 둘 다 있으면 **스킵**(idempotent).
3. `title` ← 본문 첫 H1(`# ...`)에서 `# ` 제거한 텍스트.
4. `description` ← `index.md` 에서 이 페이지를 가리키는 항목의 `— ` 뒤 요약.
   - wikilink on: `[[<basename>]]` 매칭. off: `](<상대경로>)` 매칭.
5. `title`/`description` 을 `type` 라인 바로 아래에 삽입. YAML 특수문자(`:`·`"` 등) 포함 시 큰따옴표로
   인용·이스케이프.
6. **폴백·플래그**: H1 부재 시 `title` ← 파일명 휴머나이즈(kebab→공백). `index.md` 에 항목 부재 시
   `description` 을 비워 두고 `MIGRATE-TODO` 로 stderr 에 플래그.

### 출력·검증

- stdout: 처리/스킵/플래그 건수 요약. stderr: 플래그된 페이지 목록(사람이 검토).
- 적용 후 `git diff` 로 269개 변경을 검토. 본문·기존 필드는 불변, frontmatter 2줄 삽입만 발생해야 함.
